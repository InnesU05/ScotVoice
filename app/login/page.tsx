  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignUp) {
        ...
        // CRITICAL FIX: Check if we got a session immediately
        if (data.session) {
          router.push('/dashboard'); // Auto-login enabled
        } else {
          setMessage('✅ Check your email to confirm your account!'); // Email verification still on
        }
